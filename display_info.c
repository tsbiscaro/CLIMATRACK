...
void fprint_info(info_sistema *clusters, unsigned int max_cluster,
   ...
   fprintf(fp, "Sistema | Latitude | Longitude | Tamanho | Media | Extremo | Tipo | Origem |  Vel | Direcao |\n");
   ...
   for (i = 1; i < max_cluster + 1; i++)
      ...
      switch (clusters[i].estado_separacao)
         ...
         case TIPO_SISTEMA_MERGE:
            ...
            j = 0;
            fprintf(fp, "%7d |%10.5f| %10.5f|%9d|%7.3f|%9.3f|     %s|%8d|%6.2f|%9.3f|\n",
                    i, parametros->canto_nw_y - parametros->dy*clusters[i].y_centro,
                    parametros->canto_nw_x - parametros->dx*clusters[i].x_centro,
                    (int) tamanho, clusters[i].valor_medio,
                    clusters[i].valor_extremo, tipo, clusters[i].ind_img_ant[j++],
                    vel*((parametros->dx +parametros->dy)/2)/60, ang);
         ...
         case TIPO_SISTEMA_NOVO:
            {
            fprintf(fp, "%7d |%10.5f| %10.5f|%9d|%7.3f|%9.3f|     %s|        |      |         |\n",
                    i, parametros->canto_nw_y - parametros->dy*clusters[i].y_centro,
                    parametros->canto_nw_x - parametros->dx*clusters[i].x_centro,
                    (int) tamanho, clusters[i].valor_medio,
                    clusters[i].valor_extremo, tipo);
            break;
            }
         case TIPO_SISTEMA_CONTINUACAO:
         case TIPO_SISTEMA_SPLIT:
...
void print_info(info_sistema *clusters, unsigned int max_cluster,
   ...
   printf("Sistema | Latitude | Longitude | Tamanho | Media | Extremo | Tipo | Origem |  Vel | Direcao |\n");
   if (IMGRADAR == parametros->tipo_imagem)
      {
   ...
   for (i = 1; i < max_cluster + 1; i++)
      ...
      switch (clusters[i].estado_separacao)
         ...
         case TIPO_SISTEMA_MERGE:
            ...
            printf("%7d |%10.5f| %10.5f|%9d|%7.3f|%9.3f|     %s|%8d|%6.2f|%9.3f|\n",
                   i, parametros->canto_nw_y - parametros->dy*clusters[i].y_centro,
                   parametros->canto_nw_x - parametros->dx*clusters[i].x_centro,
                   (int) tamanho, clusters[i].valor_medio,
                   clusters[i].valor_extremo, tipo, clusters[i].ind_img_ant[j++],
         ...
         case TIPO_SISTEMA_NOVO:
            {
            printf("%7d |%10.5f| %10.5f|%9d|%7.3f|%9.3f|     %s|        |      |         |\n",
                   i, parametros->canto_nw_y - parametros->dy*clusters[i].y_centro,
                   parametros->canto_nw_x - parametros->dx*clusters[i].x_centro,
                   (int) tamanho, clusters[i].valor_medio,
                   clusters[i].valor_extremo, tipo);
